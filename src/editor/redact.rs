...
//!   width/ascent/descent otherwise (see
//!   [`crate::editor::text_geometry::DEFAULT_GLYPH_WIDTH_1000`]).
...
// Font-width measurement and text/CTM matrix math are shared with
// `super::text_layout` (which needs the exact same run-bounding-box
// computation to place a selectable overlay) - see `text_geometry`'s
// module docs for why this moved out of here.
use super::text_geometry::{
    aabb, compute_show_text_advance_and_bbox, matrix_from_operands, owned_pieces_from_tj_array, rects_intersect,
    FontWidths, OwnedPiece,
};
...
// `DEFAULT_GLYPH_WIDTH_1000`/`DEFAULT_ASCENT_1000`/`DEFAULT_DESCENT_1000`
// and the `FontWidths` type they parameterize now live in
// `super::text_geometry` (imported above), shared with
// `super::text_layout`.
            ...
            match item {
                ContentItem::Op { operator, operands } => match operator.as_str() {
                    ...
                    "Tj" => {
                        ...
                        tm = new_tm;
                        if rects_intersect(target, bbox) && !pieces.is_empty() {
                            text_runs_removed += 1;
...
// `aabb`, `rects_intersect` and `matrix_from_operands` now live in
// `super::text_geometry` (imported above), shared with
// `super::text_layout`.
