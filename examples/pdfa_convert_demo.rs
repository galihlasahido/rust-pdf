...
//! - `pdfa_demo_*b.pdf`: has genuinely embedded, genuinely *subset* text
//!   (see [`build_text_document`]'s doc comment) - this used to trip
//!   [`EditableDocument::validate_pdfa`]'s `/CIDSet` check because
//!   `crate::font::cid::CompositeFont`'s subsetter didn't emit `/CIDSet`
//!   (ISO 19005-1:2005 6.3.5); fixed by making the subsetter emit it (see
//!   `crate::font::cid::CompositeFont::build`'s `cid_set` handling), so
//!   this is now expected to validate clean too.
//! - `pdfa_vector_demo_*b.pdf`: vector-only (no fonts at all), which
//!   sidesteps font embedding entirely and is kept as a minimal baseline
//!   sample alongside the font-embedding one above.
...
fn build_text_document(font_bytes: &[u8]) -> Result<Vec<u8>, Box<dyn std::error::Error>> {
    // Subsetting left at its default (enabled): `CompositeFont::build` now
    // emits the `/CIDSet` stream ISO 19005-1:2005 6.3.5 requires on
    // *subset* CID fonts (a bit vector over used CIDs, ISO 32000-1:2008
    // Table 122) - this used to be a real gap (see
    // `src/editor/pdfa.rs`'s `check_cidset_present` doc comment for the
    // history), which is exactly why this demo previously had to disable
    // subsetting to sidestep it. The heading+body text below only uses a
    // fraction of the caller-supplied font's full glyph set, so this also
    // exercises the subsetter actually reducing the font program, not just
    // a full embed with a subset-shaped name.
    let composite = CompositeFont::new(font_bytes.to_vec(), "DemoFont")?;
